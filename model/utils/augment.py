...
def augment_dataset(input_dir, output_dir, target_total, target_resolution=None):
    ...
    augmentation_pipeline = A.Compose(
        [
            A.Affine(
                scale=(0.8, 1.2), translate_percent=(-0.2, 0.2), rotate=(-45, 45), p=0.8
            ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--total", type=int, required=True, help="Target total number of images"
    )
    parser.add_argument(
        "--width", type=int, default=240, help="Target width (default: 240)"
    )
    parser.add_argument(
        "--height", type=int, default=240, help="Target height (default: 240)"
    )
    parser.add_argument(
        "--no-resize", action="store_true", help="Disable image resizing"
    )
